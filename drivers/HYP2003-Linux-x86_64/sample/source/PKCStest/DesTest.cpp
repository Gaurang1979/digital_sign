...
void DesTest::crypt_Update()
{
	...
	CK_BYTE bIn[DATA_LENGTH] = {0}, bTemp[DATA_LENGTH] = {0}, bOut[DATA_LENGTH] = {0};
	CK_ULONG ulIn = 0, ulOut = 0, ulTemp = 0;
	CK_ULONG Mechanism[3] = {CKM_DES_CBC_PAD, CKM_DES_ECB, CKM_DES_CBC};
	CK_BYTE_PTR bHint[3] = {(CK_BYTE_PTR)"CKM_DES_CBC_PAD: ",\
									(CK_BYTE_PTR)"CKM_DES_ECB: ",\
...
void DesTest::crypt_Update_CBC_PAD()
{
	const CK_ULONG DATA_LENGTH = 1024*3;
	CK_BYTE bIn[DATA_LENGTH] = {0}, bTemp[DATA_LENGTH] = {0}, bOut[DATA_LENGTH] = {0};
	CK_ULONG ulIn = 0, ulOut = 0, ulTemp = 0;
	CK_ULONG Mechanism = CKM_DES_CBC_PAD;
	CK_BYTE_PTR bHint = (CK_BYTE_PTR)"CKM_DES_CBC_PAD: ";
	SHOW_INFO("\n*	*	*	*	*	*	*	*	*	*	*	*	*	*	*	*\n");
	ulIn = 1000;
	...
	//ecnrypt init:
	CK_BYTE iv[8] = {'*','2','1','0','4','z','y','b'};
	CK_MECHANISM ckMechanism = {Mechanism, iv, sizeof(iv)};
	START_OP("Encrypting initialize.")  
	CK_RV rv =  m_gToken->C_EncryptInit(hSession, &ckMechanism, m_hKey); 
	CHECK_OP(rv);
...
void DesTest::crypt_Update_CBC()
{
	const CK_ULONG DATA_LENGTH = 1024*3;
	CK_BYTE bIn[DATA_LENGTH] = {0}, bTemp[DATA_LENGTH] = {0}, bOut[DATA_LENGTH] = {0};
	CK_ULONG ulIn = 0, ulOut = 0, ulTemp = 0;
	CK_ULONG Mechanism = CKM_DES_CBC;
	CK_BYTE_PTR bHint = (CK_BYTE_PTR)"CKM_DES_CBC: ";
	SHOW_INFO("\n*	*	*	*	*	*	*	*	*	*	*	*	*	*	*	*\n");
	ulIn = 1000;
	...
	//Get buffer's size:
	CK_ULONG ulDecrypt = 0;
	const CK_ULONG ulDec1stPice = 32;
	rv =  m_gToken->C_DecryptUpdate(hSession, bTemp, ulDec1stPice, NULL, &ulOut);
	rv =  m_gToken->C_DecryptUpdate(hSession, bTemp, ulDec1stPice, bOut, &ulOut);
	ulDecrypt +=ulOut;
	ulOut = 0;
