...
CK_MECHANISM ckMechanism1 = {CKM_SHA256_RSA_PKCS, NULL_PTR, 0};
...
CK_RV CPKCSDemo::Keypairgen() 
{
	...
	if(CKR_OK != rv)
	{
		printf("Can't Generate RSA Key Pair Error code 0x%08X\n", rv);
		return rv;
	}
	else
	{
		printf("Success Generate RSA Key Pair\n");
...
void CPKCSDemo::Encrypt() 
{
	...
	rv = C_EncryptInit(m_hSession, &ckMechanism1, m_hPubKey);
	if(CKR_OK != rv)
	{
...
void CPKCSDemo::Decrypt() 
{
	...
	CK_RV rv;
	rv = C_DecryptInit(m_hSession, &ckMechanism1, m_hPriKey);
	if(CKR_OK != rv)
	{
