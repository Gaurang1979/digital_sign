...
CK_MECHANISM ckMechanism = { CKM_ECDSA_SHA256, NULL_PTR, 0 };
CK_BYTE pbMsg[] = "hyp2003 bring you into e-Business time.";
...
CK_RV CPKCSDemo::Login()
{
	...
	printf("You Must input user pin before generating ECC Key pair\n");
	char s[32];
	scanf("%s",s);
	g_strUserPIN = s;
...
CK_RV CPKCSDemo::GetMechanismList()
{
	CK_RV rv = 0;
	CK_ULONG lCount = 100;
	CK_MECHANISM_TYPE_PTR  pMechanismList = (CK_MECHANISM_TYPE_PTR) new CK_MECHANISM_TYPE[lCount];
	rv = C_GetMechanismList(m_pSlotList[0], pMechanismList, &lCount);
	if(CKR_OK != rv)
	{
		printf("\nGetMechanismList failed ,Error code 0x%08X\n", rv);
		return rv;
    }
	printf("The GetMechanismList count = %ld\n", lCount);
...
CK_RV CPKCSDemo::Keypairgen() 
{
	...
	if(CKR_OK != rv)
	{
		printf("Can't Generate ECC Key Pair Error code 0x%08X\n", rv);
		return rv;
	}
	else
	{
		printf("Success Generate ECC Key Pair\n");
...
void CPKCSDemo::Encrypt() 
{
	...
	printf("\nNow You can decrypt use ECC private key\n");
}
